...
import { getDoctorProfile, uploadFirebase } from "./api";
  ...
  const props = {
    ...
    onChange(info) {
      if (info.file.status === "done") {
        ...
        reader.onloadend = () => {
            dispatch(uploadFirebase(info, token, doctorId));
        }
      ...
      >
        <Col span={20}>
          <Card className="profil-card">
            ...
            {doctor ? <ProfileForm biodata={doctor} /> : <Skeleton active />}
          </Card>
        </Col>
      </Row>
    </Layout>
